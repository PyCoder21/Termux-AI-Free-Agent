...
- Для любых команд требующих взаимодействия пользователя (интерактивные) - используй run_cmd_pexpect, он позволяет выполнять команды ПОЛНОСТЬЮ интерактивно, даже очень интерактивные, но не программы с curses, так как это ломает терминал.
"""
